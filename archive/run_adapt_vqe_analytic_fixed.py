# run_adapt_vqe_analytic_fixed.py — ADAPT-VQE s presnými analytickými gradientmi
...
def main():
    ...
    def obj_and_grad(th):
        ...
        for i in reversed(range(len(th))):
            A = pool[sel[i]]
            # OPRAVA: Použijeme v_list[i+1], teda stav po prechode bránou
    ...
    for it in range(1, MAXOPS+1):
        ...
        kbest = int(np.argmax(pool_grads)); gmax = float(pool_grads[kbest])
        ...
        res = minimize(obj_and_grad, theta, method="BFGS", jac=True, options={"gtol": 1e-5, "maxiter": 200})
