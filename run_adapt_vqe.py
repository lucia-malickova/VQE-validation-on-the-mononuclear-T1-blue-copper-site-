# run_adapt_vqe.py — spravny ADAPT-VQE (GLOBALNA re-optimalizacia vsetkych uhlov kazdy krok)
...
def main():
    ...
    sel = []                      # poradie vybranych operatorov v ansatze
    theta = np.zeros(0)
    ...
    for it in range(1, MAXOPS+1):
        ...
        # >>> KLUCOVY ROZDIEL: preoptimalizuj VSETKY uhly, nie len ten novy <
        res = minimize(energy, theta, method="BFGS", options={"gtol": 1e-6, "maxiter": 1000})
        theta = res.x; E = float(res.fun); dev = (E - E0) * 1e3
        hist.append((len(sel), E, dev, gmax))
        print(f"iter {it:3d}  ops={len(sel):3d}  add={labels[kbest]}  E={E:.6f}  dev={dev:9.3f} mEh  gmax={gmax:.2e}", flush=True)
        if dev < CHEM:
            print(f"\nSUCCESS: chemicka presnost ({dev:.3f} mEh) za {len(sel)} operatorov, {time.time()-t0:.0f}s", flush=True); break

    nS = sum(1 for k in sel if labels[k][0] == "S"); nD = sum(1 for k in sel if labels[k][0] == "D")
    print(f"[circuit] aplikacii={len(sel)} (S={nS}, D={nD})  ~2q brany≈{2*nS+13*nD}  params={len(sel)}", flush=True)
