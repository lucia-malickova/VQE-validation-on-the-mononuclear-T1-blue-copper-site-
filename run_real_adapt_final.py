# run_real_adapt_final.py — Víťazný ADAPT-VQE (K_ADD=8, zlom symetrie + ukladanie)
...
def main():
    ...
    print(f"Pool = {len(pool)} operatorov (Iba S+D)", flush=True)
    ...
    # Pôvodné víťazné nastavenia: K_ADD=8, MAX_ITERS=50
    CHEM = 1.6; MAX_ITERS = 50; K_ADD = 8
    hist = []; t0 = time.time()
    ...
    for it in range(1, MAX_ITERS+1):
        ...
        # KĽÚČ: Zlom symetrie, inak to uviazne na 106 mEh!
        theta = np.append(theta, np.random.uniform(-0.001, 0.001, K_ADD))
        
        # Prísnejší L-BFGS-B z víťazného behu
    ...
    # UKLADANIE DÁT DO NPZ SÚBORU (Toto tu včera chýbalo)
